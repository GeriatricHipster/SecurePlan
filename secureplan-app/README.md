...
- All survey folders and their plans populate as soon as a site opens, with optional folder filters instead of required folder-by-folder navigation
...
- Direct blueprint panning in Select mode with mouse, trackpad, stylus, or one-finger mobile touch, plus anchored two-finger pinch zoom on mobile
- Blue-by-default device icons with a manual color picker that also recolors the supplied Access Control artwork
- Color-coded installation progress: Planned slate, Ready blue, In Progress yellow, Installed purple, Tested teal, Complete green, and Blocked red
- Door functions for Monitored, Controlled, and Full openings with automatic tan, blue, and green icon colors while installation status remains a separate colored dot
...
- Streamlined responsive layouts with compact mobile survey rows, horizontal folder filters, editor bottom sheets, and installable PWA metadata
